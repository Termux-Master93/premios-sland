...
export const VoteFnal = (
    ...
    const [result, setResults] = useState(RESULT_STATUS.IDLE)
    useEffect(() => {
        window.scrollTo({ top: 0, behavior: 'smooth' })
    }, [])

    const selectCandidates = useMemo(() => {
        const results: Record<string, typeof CandidatesType[0]['candidates'][0]> = {}
        ...
        <section class="flex flex-col">
            ...
            </h1>
            {
                result === RESULT_STATUS.ERROR && (
                    'Hubo un error a enviar tus votos, intenta nuevamente'
                )
            }

            {
                (result === RESULT_STATUS.IDLE || isLoading) && (
                    <>
                        <h1
                            class="pt-24 pb-10 mx-auto text-balance text-left text-3xl lg:text-5xl font-semibold tracking-wide"
                        >
                            Tus votos Finales
                        </h1>
                        <div class={`${isLoading ? 'opacity-50' : ''} transition grid grid-cols-2 md:grid-cols-6 gap-2 p-4`}>
                            {
                                votes.map((categoryVotes, index) => {
                                    return (
                                        <button
                                            class="bg-[#1682c7] hover:scale-105 hover:bg-[#54beff] flex flex-col justify-between items-center" onClick={() => setCategory(index)}
                                        >
                                            <ul class="grid grid-cols-2">
                                                {
                                                    categoryVotes.map((candidate) => {
                                                        const info = selectCandidates?.[candidate]
                                            ...
                                            <h2 class="text-lg font-semibold my-auto text-balance">
                                                {categoryNames[index]}
                                            </h2>
                                        </button>
                                    )
